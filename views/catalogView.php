     ...
     <div class="container">
            ...
            <form name="page_tri" id="page_tri" action="" method="post">
                ...
                <select id="choix_page" name="choix_page" class="form-control">
                    <?php for ($i = 1; $i <= $pages; $i++) : ?>
                        <option value="<?= $i ?>"<?php if ($i === $page) : ?>selected="selected"><?= $i ?><?php else : ?>><?= $i ?><?php endif; ?></option>
                    <?php endfor; ?>
                </select>
            ...
            <table class="table-responsive">
                ...
                <?php foreach ($dp_sqlShowCatalog as $value) : ?>
                    <tr>
                        ...
                        <td style="padding-left: 15px"><a href="movie.php?id=<?= $value['ID'] ?>"><input type="button" class="btn btn-primary" name="detail" value="Détails" /><br /><br /></a><a href="admin/movies.php?id=<?= $value['ID'] ?>"><input type="button" class="btn btn-primary" name="modifier" value="Modifier" /></a></td>
                    </tr>
            	<?php endforeach; ?>
            </table>
        </div>
        ...
        <nav aria-label="pagination">
            <ul class="pager">
                <?php if ($page > 1) : ?>
                <li><a href="?page=<?= $page -1 ?><?php if ($triDate) : ?>&tri=<?= $triDate ?><?php endif; ?><?php if ($cat) : ?>&cat=<?= $cat ?><?php endif; ?>"><span aria-hidden="true">&larr;</span> Précédent</a></li>
                <?php endif; ?>
                <li> Page <?= $page ?> de <?= $pages ?> </li>
                <?php if ($page < $pages) : ?>
                <li><a href="?page=<?= $page + 1 ?><?php if ($triDate) : ?>&tri=<?= $triDate ?><?php endif; ?><?php if ($cat) : ?>&cat=<?= $cat ?><?php endif; ?>">Suivant <span aria-hidden="true">&rarr;</span></a></li>
                <?php endif; ?>
            </ul>
        </nav>
    </div>